...
def main() -> None:
    ...
    converter = Converter(input_path, output_file)
    if converter.convert():
        logging.info("Processed file saved to %s", output_file.absolute())
    else:
        logging.error("An error occurred while processing the LUIS JSON file: %s", input_path.absolute())
        sys.exit(1)
